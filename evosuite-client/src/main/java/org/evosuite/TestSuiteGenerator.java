...
public class TestSuiteGenerator {
	 ...
	 */
	public TestGenerationResult generateTestSuite(String tDir, String rDir) {
		...
		if (ClientProcess.DEFAULT_CLIENT_NAME.equals(ClientProcess.getIdentifier())) {
			TestSuiteChromosome oldTestCases = testCases.clone();
			
			oldTestCases.getTestChromosomes()
			...
			result = writeJUnitTestsAndCreateResult(testCases, oldTestCases, tDir, rDir);
	...
	private static void writeJUnitTestSuiteForFailedInitialization() throws EvosuiteError {
		...
		DefaultTestCase test = buildLoadTargetClassTestCase(Properties.TARGET_CLASS);
		suite.addTest(test);
		writeJUnitTestsAndCreateResult(suite, null, null, null);
	}
	 ...
	 *            a test suite.
	 */
	public static TestGenerationResult writeJUnitTestsAndCreateResult(TestSuiteChromosome testSuite, TestSuiteChromosome nrSuite, String suffix, String tDir, String rDir) {
		List<TestCase> tests = testSuite.getTests();
		List<TestCase> nrTests = null;
		if(nrSuite != null)
			nrTests = nrSuite.getTests();
		...
		if (Properties.JUNIT_TESTS) {
			...
			LoggingUtils.getEvoLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Writing JUnit test case '"
					+ (name + suffix) + "' to " + tDir);
			suiteWriter.writeTestSuite(name + suffix, tDir, testSuite.getLastExecutionResults());

			if(nrTests != null){
				TestSuiteWriter nrSuiteWriter = new TestSuiteWriter();
				nrSuiteWriter.insertTests(nrTests);
				LoggingUtils.getEvoLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Writing JUnit test case '"
						+ (name + "NR" + suffix) + "' to " + rDir);
				nrSuiteWriter.writeTestSuite(name + "NR" + suffix, rDir, nrSuite.getLastExecutionResults());
			}
		}
	 ...
	 * @param tDir 
	 */
	public static TestGenerationResult writeJUnitTestsAndCreateResult(TestSuiteChromosome testSuite, TestSuiteChromosome nrTestSuite, String tDir, String rDir) {
		return writeJUnitTestsAndCreateResult(testSuite, nrTestSuite, Properties.JUNIT_SUFFIX, tDir, rDir);
	}
	...
	public static void main(String[] args) {
		...
		generator.generateTestSuite(null, null);
		System.exit(0);
	}
